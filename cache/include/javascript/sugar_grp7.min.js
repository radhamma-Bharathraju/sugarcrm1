...
var header=app.additionalComponents['header-nav'];var quicksearch=header&&header.getComponent('quicksearch');if(quicksearch){quicksearch.trigger('route:search');}
if(appContext&&appContext.get('search')){return;}
app.controller.loadView({layout:'search',searchTerm:searchTerm,module_list:params.modules,tagParams:params.tags,mixed:true});}},{name:'list',route:':module',callback:function(module){if(!app.router._moduleExists(module)){return;}
const enabledModulesKey='enabled_modules';let pipelineConfig=app.metadata.getModule('VisualPipeline','config')||{};let enableModules=pipelineConfig[enabledModulesKey]||[];let isAvailablePipeline=_.includes(enableModules,module);let layout='records';if(app.user.lastState.get(module+':pipeline')){if(isAvailablePipeline){layout='pipeline-records';}else{app.user.lastState.set(module+':pipeline',0);app.user.lastState.set(`${module}:list-filterpanel:toggle-view`,'list');}}
app.controller.loadView({module:module,layout:layout});}},{name:'create',route:':module/create',callback:function(module){if(!app.router._moduleExists(module)){return;}
...
else if(Math.abs(d)<1000){d=app.utils.charts.round(d,p);}else if(Math.abs(d)>=1e9){d /=1e9;f=typeof l==='undefined'?d3sugar.format:d3sugar.formatLocale(l).format;return f('s')(d)+'B';}else{f=typeof l==='undefined'?d3sugar.formatPrefix:d3sugar.formatLocale(l).formatPrefix;if(p!==0){var d1=f('.'+p+'s',d)(d);var d2=d1.split('.').pop().replace(/[^\d]+$/,'').match(/0+$/g);if(Array.isArray(d2)){p-=d2.pop().length;}}
...
(function(app){var _useRequiredLabels={"bool":true,"radioenum":'edit'};app.events.on("app:init",function(){var _fieldProto=_.clone(app.view.Field.prototype);_.extend(app.view.Field.prototype,{hideHelp:false,exclamationMarkTemplate:Handlebars.compile(`<span class="error-tooltip add-on" data-container="body" rel="tooltip" title="{{arrayJoin this ", "}}">
                 ...
                 </span>`),handleValidationError:function(errors){this.clearErrorDecoration();_.defer(function(field){field._errors=errors;if(_.include(['edit','disabled'],field.action)||field.view.inlineEditMode){if(field.parent){field.parent.setMode('edit');}else{field.setMode('edit');}}
...
this._addViewClass(this.action);if(!_.isEmpty(this._errors)){if(_.include(['edit','disabled'],this.action)){this.decorateError(this._errors);}}
if(this.def.required){this.clearRequiredLabel();if((this.action==='edit'||-1!==_.indexOf(['edit','list-edit','disabled'],this.tplName))&&this._shouldRenderRequiredPlaceholder()){this.decorateRequired();}}
...
if(field.type==='htmleditable_tinymce'){field.render();return new Handlebars.SafeString(field.$el.html());}
...
(function(app){app.events.on('app:init',function(){var getSystemActions=function(){var actions=[{callback:'openConfig',action:'config',name:app.lang.get('LBL_SWEETSPOT_CONFIG'),icon:'sicon-settings'}];return actions;};var hasAccessToAction=function(module,action){if(module&&action.acl_action){if(!app.acl.hasAccess(action.acl_action,module)){return false;}
return action;}
...
(function utils(app){app.Calendar=app.Calendar||{};let utils={};utils.buildUserKeyForStorage=function buildKeyForStorage(componentName){return'Calendar:'+app.user.id+':'+componentName+':storage';};utils.buildUserKeyForShowFilteredRecords=function buildKeyForStorage(componentName){return'Calendar:'+app.user.id+':'+componentName+':applyFilter';};utils.getConfigurationsByKey=function getConfigurationsByKey(key,calendarCategory){let calendarsSaved=app.user.lastState.get(key);let calendarConfigurations={myCalendars:[],otherCalendars:[]};if(typeof calendarsSaved==='object'){if(!_.isEmpty(calendarCategory)){return calendarsSaved[calendarCategory];}
...
self.$('li[data-bs-toggle="tab"]').on('shown.bs.tab',function(e){var tabName=$(e.target.parentElement).data('tab-name');$navTabs=$(e.target).parents('.nav-tabs');$tabbableClass=$(e.target).parents('.tabbable');$navTabs.addClass('preview-pane-tabs',tabName==='preview');$tabbableClass.addClass('preview-active',tabName==='preview');});}
...
if(_.has(customOptions,'filtersDef')&&customOptions.filtersDef){filters=this._applyDashboardFilters(customOptions.filtersDef);}
if(!stateKey){return filters;}
const lastState=app.user.lastState.get(stateKey);if(_.has(lastState,'filtersDef')){filters=this._applyDashboardFilters(lastState.filtersDef);}
return filters;},_applyDashboardFilters:function(filters){const customFilters=app.utils.deepCopy(filters);const dashletFilters=customFilters.Filter_1;const customContainer=this.closestComponentByType(this.layout,'dashlet-grid-wrapper');let dashboardFilterGroups=customContainer.model.get('metadata').filters;const dashletId=customContainer.el.getAttribute('data-gs-id');const dashboardId=customContainer.model.get('id');const userDashboardFiltersSetup=this._getUserDashboardLastState(dashboardId);if(!_.isEmpty(userDashboardFiltersSetup)){dashboardFilterGroups=userDashboardFiltersSetup.filters;}
_.each(dashboardFilterGroups,(dashboardFilterGroup)=>{_.each(dashboardFilterGroup.fields,_.bind(this._applyFiltersFields,this,dashletId,dashletFilters,dashboardFilterGroup));});return customFilters;},_applyFiltersFields:function(dashletId,dashletFilters,dashboardFilterGroup,dashboardFilterField){_.each(dashletFilters,(dashletFilter,dashletFilterKey)=>{if(dashletFilterKey==='operator'){return;}
if(!_.isEmpty(dashletFilter.operator)){this._applyFiltersFields(dashletId,dashletFilter,dashboardFilterGroup,dashboardFilterField);}else if(dashboardFilterField.dashletId===dashletId&&dashboardFilterField.fieldName===dashletFilter.name&&dashboardFilterField.tableKey===dashletFilter.table_key){const inputValues=4;for(let inputValueIdx=0;inputValueIdx<inputValues;inputValueIdx++){dashletFilter[`input_name${inputValueIdx}`]='';}
_.each(dashboardFilterGroup.filterDef,(propertyValue,propertyKey)=>{dashletFilter[propertyKey]=propertyValue;});}});},closestComponentByType:function(container,type){if(!container){return;}
if(container.type===type){return container;}
return this.closestComponentByType(container.layout,type);},_getUserDashboardLastState:function(dashboardId){const userDashboardStateKey=this._getUserDashboardLastStateKey(dashboardId);const dashboardFiltersStored=app.user.lastState.get(userDashboardStateKey);return dashboardFiltersStored;},_getUserDashboardLastStateKey:function(dashboardId){const dashboardKey=`Dashboards:${dashboardId}`;const dashboardComponentKey='dashboard-filters';const lastStateKey=app.user.lastState.buildKey(dashboardComponentKey,app.user.id,dashboardKey);return lastStateKey;},_getCustomReportMeta:function(customReportDef,lastStateKey){let customMeta={};if(customReportDef){if(_.has(customReportDef,'summaryColumns')&&!_.isEmpty(customReportDef.summaryColumns)){customMeta.summaryColumns=customReportDef.summaryColumns;}
...
customMeta.filtersDef=this._getCustomFiltersMeta(customReportDef,lastStateKey);return customMeta;},_adjustLoadingWidgetSize:function(loadingElementType){if(this.disposed){return;}
const loadingWidget=this.$el.parentsUntil('.grid-stack-item').find(`[data-widget=report-loading][data-type=${loadingElementType}]`);const loadingWidgetParent=loadingWidget.parent();const loadingWidetParentWidth=loadingWidgetParent.width();const loadingWidetParenHeight=loadingWidgetParent.height();if(loadingWidetParentWidth&&loadingWidetParentWidth>0&&loadingWidetParenHeight>0){loadingWidget.width(loadingWidetParentWidth);loadingWidget.height(loadingWidetParenHeight);}},_showLoadingScreen:function(show,type=false){if(this.disposed){return;}
let loadingEl;if(type){loadingEl=this.$el.parentsUntil('.grid-stack-item').find(`[data-widget=report-loading][data-type=${type}]`);}else{loadingEl=this.$el.parentsUntil('.grid-stack-item').find('[data-widget=report-loading]');}
if(show&&_.isBoolean(show)){loadingEl.show();if(_.isFunction(this._showAdditionalComponents)){this._showAdditionalComponents();}}else{loadingEl.hide();if(_.isFunction(this._hideAdditionalComponents)){this._hideAdditionalComponents();}
this.context.trigger('refresh-results-complete');}
if(!show){this.context.trigger('dashlet-finished-loading');}},});});})(SUGAR.App);
/* End of File include/javascript/sugar7/plugins/ReportsPanel.js */

(function(app){app.events.on('app:init',function(){app.plugins.register('ReportExport','view',{exportToPdf:function(options){if(!options){options={};}
let queryParams={};const reportData=this.context.get('data');const hasChart=this.context.get('reportHasChart');if(reportData&&_.has(reportData,'orderBy')){queryParams.orderBy=reportData.orderBy;}
if(hasChart){queryParams.shouldHaveChartCanvas=true;}
let reportId=this.context.get('model').get('id');if(!reportId&&this.layout&&_.has(this.layout,'model')){reportId=this.layout.model.get('report_id');}
let url=app.api.buildURL('Reports',reportId+'/'+'base64',null,queryParams);app.alert.show('export-to-pdf',{level:'process',title:app.lang.get('LBL_EXPORT_PDF','Reports'),});this.$('[data-content=report-export-modal]').hide();const completeCallback=options.completeCallback||this.exportCompleteCallback;const errorCallback=options.exportErrorCallback||this.exportErrorCallback;app.api.call('read',url,{},{complete:_.bind(completeCallback,this,'pdf'),error:_.bind(errorCallback,this,'pdf'),});},exportToCsv:function(options){if(!options){options={};}
let queryParams={};const reportData=this.context.get('data');if(reportData&&_.has(reportData,'orderBy')){queryParams.orderBy=reportData.orderBy;}
let reportId=this.context.get('model').get('id');if(!reportId&&this.layout&&_.has(this.layout,'model')){reportId=this.layout.model.get('report_id');}
...
let blob=new Blob([arr],{type:'application/octet-stream',});if(contentType==='csv'){const bom=new Uint8Array([0xEF,0xBB,0xBF]);blob=new Blob([bom,binStr],{type:'text/plain;charset=utf-8',});}
return blob;};const blob=dataURIToBlob(content);const url=URL.createObjectURL(blob);let element=document.createElement('a');element.setAttribute('href',url);element.setAttribute('download',filename);element.style.display='none';document.body.appendChild(element);element.click();document.body.removeChild(element);},formatDateToString:function(){const currentDate=moment();return currentDate.format('YYYY_MM_DD_HH_mm');},exportErrorCallback:function(type,error){if(error&&(error.errorThrown==='timeout'||error.textStatus==='timeout')){app.alert.show(`export-to-${type}-failed`,{level:'error',messages:app.lang.get('LBL_EXPORT_FAILED','Reports'),});if(_.isFunction(this.closeModal)){this.closeModal();}
app.alert.dismiss(`export-to-${type}-failed`);}},exportCompleteCallback:function(type,data){if(this.disposed){return;}
const reportModelName=this._buildReportName();const currentTime=this.formatDateToString();const userName=app.user.get('user_name');const reportName=reportModelName.replace(/ /g,'_');const name=`${reportName}_${userName}_${currentTime}.${type}`;this.downloadFileLocally(name,data.xhr.responseText,type);if(_.isFunction(this.closeModal)){this.closeModal();}
const typeUpper=type.toUpperCase();app.alert.dismiss(`export-to-${type}`);app.alert.show(`export-to-${type}-success`,{level:'success',messages:app.lang.get(`LBL_EXPORT_${typeUpper}_SUCCESS`,'Reports'),autoClose:true});},_buildReportName:function(){let reportModelName=this.context.get('model').get('name');if(!reportModelName&&this.layout){reportModelName=this.layout.model.get('report_name');}
if(!reportModelName){reportModelName='Report';}
return reportModelName;},});});})(SUGAR.App);
/* End of File include/javascript/sugar7/plugins/ReportExport.js */

(function DocuSignConsole(app){const utils={initiateSend:function(payload,step){if(_.isUndefined(payload.processInProgress)&&app.DocuSign._activeSendingProcess===true){return;}
app.DocuSign._activeSendingProcess=true;payload.processInProgress=true;const showRecipients=this.shouldShowRecipients();if(_.isUndefined(step)){if(showRecipients){let contextModule=this._getEnvelopeSourceModule();let ctxModelId=this._getEnvelopeSourceModelId();let recipientsListContext={module:'DocuSignEnvelopes',contextModule:contextModule,ctxModelId:ctxModelId,isMultiSelect:true,};if(payload.template){recipientsListContext.templateDetails=payload.template;}
app.drawer.open({layout:'recipients-list',type:'recipients-list',context:recipientsListContext},_.bind(function(selected){if(selected.closeEvent){app.DocuSign._activeSendingProcess=false;return;}
payload.recipients=[];_.each(selected,function(recipient){payload.recipients.push(recipient);});const nextStep='envelopeSetup';this.initiateSend(payload,nextStep);},this));return;}else{const nextStep='envelopeSetup';this.initiateSend(payload,nextStep);return;}}
if(step==='envelopeSetup'){app.drawer.open({layout:'envelope-setup',type:'envelope-setup',context:{module:'DocuSignEnvelopes',payload:payload,}},_.bind(function(envelopeDetails){if(envelopeDetails.closeEvent){if(_.isUndefined(envelopeDetails.dsProcessWillContinue)){app.DocuSign._activeSendingProcess=false;}
return;}
const step='openEnvelope';payload.envelopeName=envelopeDetails.envelopeName;this.initiateSend(payload,step);},this));return;}
if(step==='openEnvelope'||step==='openDraft'){app.DocuSign._activeSendingProcess=false;const tab=window.open(app.DocuSign._loadPageUrl);this.openEnvelope(payload,tab);return;}
if(step==='selectTemplate'){app.drawer.open({layout:'templates-list',type:'templates-list',context:{module:'DocuSignEnvelopes',}},_.bind(function(selected){if(selected.closeEvent){app.DocuSign._activeSendingProcess=false;return;}
payload.templateSelected=selected;const step='getTemplate';this.initiateSend(payload,step);},this));return;}
if(step==='getTemplate'){app.api.call('create',app.api.buildURL('DocuSign/getTemplateDetails'),{template:payload.templateSelected},{success:_.bind(function(template){payload.template=template;this.initiateSend(payload);},this),});return;}},initiateCompositeSend:function(payload,step){if(_.isUndefined(payload.processInProgress)&&app.DocuSign._activeSendingProcess===true){return;}
app.DocuSign._activeSendingProcess=true;payload.processInProgress=true;const showRecipients=this.shouldShowRecipients();if(_.isUndefined(step)&&showRecipients){let contextModule=this._getEnvelopeSourceModule();let ctxModelId=this._getEnvelopeSourceModelId();let recipientsListContext={module:'Users',contextModule:contextModule,ctxModelId:ctxModelId,};if(payload.template){recipientsListContext.templateDetails=payload.template;}
app.drawer.open({layout:'recipients-list-composite',type:'recipients-list-composite',context:recipientsListContext,isMultiSelect:true},_.bind(function(selected){if(_.isUndefined(selected)||selected.closeEvent){if(_.isUndefined(selected.dsProcessWillContinue)){app.DocuSign._activeSendingProcess=false;}
return;}
payload.recipients=[];_.each(selected,function(recipient){payload.recipients.push(recipient);});const nextStep='envelopeSetup';payload.composite=true;this.initiateSend(payload,nextStep);},this));return;}
if((_.isUndefined(step)&&!showRecipients)||step==='envelopeSetup'){const nextStep='envelopeSetup';this.initiateSend(payload,nextStep);return;}
if(step==='selectTemplate'){app.drawer.open({layout:'templates-list-composite',type:'templates-list-composite',context:{module:'DocuSignEnvelopes',}},_.bind(function(selected){if(_.isUndefined(selected)||selected.closeEvent){app.DocuSign._activeSendingProcess=false;return;}
payload.templateSelected=selected;payload.composite=true;const step='getTemplate';this.initiateCompositeSend(payload,step);},this));return;}
if(step==='getTemplate'){app.api.call('create',app.api.buildURL('DocuSign/getTemplateDetails'),{template:payload.templateSelected},{success:_.bind(function(template){payload.template=template;this.initiateCompositeSend(payload);},this),});return;}
const nextStep='openEnvelope';this.initiateCompositeSend(payload,nextStep);},openEnvelope:function(payload,tab){app.alert.show('load-tab-for-sending',{level:'process',title:app.lang.get('LBL_LOADING')});app.api.call('create',app.api.buildURL('DocuSign/send'),payload,{success:_.bind(this.succesGettingEnvelope,this,tab),error:function(error){app.alert.show('error-loading-tab',{level:'error',messages:error.message});},complete:function(){app.alert.dismiss('load-tab-for-sending');}});},isRecipientSelectionEnabled:function(){let recipientSelectionIsEnabled=false;if(!_.isUndefined(app.config.docusign)&&_.isString(app.config.docusign.recipientSelection)){recipientSelectionIsEnabled=app.config.docusign.recipientSelection==='show';}
return recipientSelectionIsEnabled;},shouldShowRecipients:function(){const ctxModel=app.controller.context;const recordViewContext=ctxModel.get('layout')==='record';const homeModule=ctxModel.get('module')==='Home';const recipientSelectionIsEnabled=this.isRecipientSelectionEnabled();const showRecipients=recipientSelectionIsEnabled&&recordViewContext&&!homeModule;return showRecipients;},succesGettingEnvelope:function(tab,res){if((res.status&&res.status==='error')||res.envelopeStatus==='deleted'){var minifiedErrorMessage=res.message.toLowerCase();if(minifiedErrorMessage.includes('envelope status in docusign is now')){if(res.envelopeStatus==='deleted'){this.confirmDelete(res);}else{this.confirmUpdate(res);}}else if(minifiedErrorMessage!=='cancel'&&!_.isEmpty(res.message)){app.alert.show('ds_error',{level:'error',messages:res.message,autoClose:false});}
tab.close();return;}
tab.location.href=res.url;this.listenForTabClosing();},confirmDelete:function(data){app.alert.show('draft-does-not-exist',{level:'confirmation',messages:app.lang.get('LBL_DRAFT_DELETED_CONFIRMATION','DocuSignEnvelopes'),autoClose:false,onConfirm:function(){let removeCallbacks=this.getConfirmationCallbacks({successMessage:app.lang.get('LBL_DRAFT_DELETE_SUCCESS','DocuSignEnvelopes'),errorMessage:app.lang.get('LBL_DRAFT_DELETE_ERROR','DocuSignEnvelopes')});app.api.call('create',app.api.buildURL('DocuSign/removeEnvelope'),{envelopeId:data.envelopeId},removeCallbacks);app.alert.show('envelope-loading',{level:'process',title:app.lang.get('LBL_LOADING')});},});},confirmUpdate:function(data){app.alert.show('draft-does-not-exist',{level:'confirmation',messages:app.lang.get('LBL_DRAFT_CHANGED_CONFIRM','DocuSignEnvelopes',{status:data.status}),autoClose:false,onConfirm:function(){let updateCallbacks=this.getConfirmationCallbacks({successMessage:app.lang.get('LBL_DRAFT_CHANGED_SUCCESS','DocuSignEnvelopes'),errorMessage:app.lang.get('LBL_DRAFT_CHANGED_ERROR','DocuSignEnvelopes')});app.api.call('create',app.api.buildURL('DocuSign/updateEnvelope'),{envelopeId:data.envelopeId},updateCallbacks);app.alert.show('envelope-loading',{level:'process',title:app.lang.get('LBL_LOADING')});},});},getConfirmationCallbacks:function(options){return{success:function(res){if(res){app.alert.show('succes-change-envelope',{level:'success',messages:options.successMessage,autoClose:true});app.events.trigger('docusign:reload');}else{app.alert.show('error-change-envelope',{level:'error',messages:options.errorMessage,autoClose:true,autoCloseDelay:'10000'});}},error:function(error){app.alert.show('error-change-envelope',{level:'error',messages:error.message});},complete:function(){app.alert.dismiss('envelope-loading');}};},listenForTabClosing:function(){$(window).on('storage.docusignAction',function(e){if(e.originalEvent.key!=='docusignAction'){return;}
const action=e.originalEvent.newValue;if(!action){return;}
$(window).off('storage.docusignAction');app.events.trigger('docusign:send:finished');if(app.controller.context.get('module')==='pmse_Inbox'&&app.controller.layout.name==='show-case'){return;}
...
return module;},_getEnvelopeSourceModelId:function(){var module=app.controller.context.get('module');var modelId=app.controller.context.get('modelId');if(module==='pmse_Inbox'&&app.controller.layout.name==='show-case'){try{var sourceModel=app.controller.layout._components[0].getComponent('sidebar').getComponent('main-pane').model;return sourceModel.get('id');}catch(showCaseError){app.log.error(`_getEnvelopeSourceModelId. show-case layout error: ${showCaseError}`);}}
return modelId;},_showRolesNotSetAlert:function(){const msg=app.lang.get('LBL_RECIPIENT_ROLE_MISSING_ERROR','DocuSignEnvelopes');app.alert.show('recipient-role-not-set',{level:'error',messages:msg,autoClose:true});},};app.DocuSign=app.DocuSign||{};app.DocuSign.utils=utils;app.events.on('app:init',function(){app.DocuSign._loadPageUrl=app.api.buildURL('DocuSign','loadPage');app.events.on('docusign:send:initiate',app.DocuSign.utils.initiateSend,app.DocuSign.utils);app.events.on('docusign:compositeSend:initiate',app.DocuSign.utils.initiateCompositeSend,app.DocuSign.utils);app.events.on('app:view:change',function(layoutType,contextOptions){const docusignSendingLayouts=['recipients-list','recipients-list-composite','templates-list','templates-list-composite','envelope-setup'];if(!_.contains(docusignSendingLayouts,layoutType)&&_.isUndefined(contextOptions.drawer)){app.DocuSign._activeSendingProcess=false;return;}});});})(SUGAR.App);
/* End of File include/javascript/docusign/docusign-console.js */

(function(app){app.events.on('app:init',function(){app.plugins.register('ReportIntelligenceDashlet','view',{_setIntelligence:function(){const intelligenceEl=this.$('[data-fieldname="intelligent"]');const checkboxElement=this.$('[data-fieldname="intelligent"] input');const field=this.getField('linkedFields');if(!field){return;}
const fieldEl=this.$('[data-name="linkedFields"]');if(checkboxElement.is(':checked')==1){fieldEl.show();}else{fieldEl.hide();}
if(this._isNotRecordLayout()&&this._isNotFocusDrawerLayout()){intelligenceEl.parent().hide();fieldEl.parent().hide();}},_setLinkedFields:function(reportModule){if(!this.meta.config){return;}
reportModule=reportModule?reportModule:this.settings.get('module');const linkedField=this.getField('linkedFields');if(!linkedField){return;}
let relationships=this._getRelationships(reportModule);linkedField.items=relationships;if(_.isEmpty(linkedField.items)){this._toggleIntelligence(false);this.settings.set('linkedFields','');return;}
let linkName=this.settings.get('linkedFields');const link=linkedField.items[linkName];if(_.isUndefined(link)){this.settings.set('linkedFields','');}
linkedField._render();if(!linkName&&this.settings.get('intelligent')){linkName=_.chain(relationships).keys().first().value();this.settings.set('linkedFields',linkName);linkedField.$('.select2-container').select2('data',{id:linkName,text:relationships[linkName],});}},_toggleInput:function(enable,fieldName,title){if(!this.meta.config){return;}
const checkboxElement=this.$(`[data-fieldname="${fieldName}"] input`);$(checkboxElement).prop('disabled',!enable);if(!enable){this.settings.set(fieldName,false);}
this._toggleInputTooltip(!enable,fieldName,title);},_toggleIntelligence:function(enable){if(!this.meta.config){return;}
const checkboxElement=this.$('[data-fieldname="intelligent"] input');$(checkboxElement).prop('disabled',!enable);if(!enable){const fieldEl=this.$('[data-name="linkedFields"]');$(checkboxElement).prop('checked',false);fieldEl.hide();this.settings.set({linkedFields:'',intelligent:false,});}
this._toggleIntelligenceTooltip(!enable);},_wrapInputEl:function(fieldName){const containerFieldName=`${fieldName}Container`;let checkboxEl=this.$(`[data-fieldname="${fieldName}"] input`);let checkboxContainer=`<div class="saved-report-tooltip" data-fieldname="${containerFieldName}"`+` data-container="body">`;if(this.$(`[data-fieldname="${containerFieldName}"]`).length>0){return;}
checkboxEl.parent().prepend(checkboxContainer);let checkboxContainerEl=this.$(`[data-fieldname="${containerFieldName}"]`);checkboxContainerEl.append(checkboxEl.detach());},_toggleIntelligenceTooltip:function(enable){const sideCtx=app.sideDrawer.currentContextDef;const appContext=app.controller.context;const module=sideCtx?sideCtx.context.module:appContext.get('module');const message=app.lang.get('LBL_REPORTS_DASHLET_NO_LINKS_SELECTED',null,{module:module,});this._toggleInputTooltip(enable,'intelligent',message);},_toggleInputTooltip:function(enable,fieldName,title){let checkboxContainerEl=$(`[data-fieldname="${fieldName}Container"]`);checkboxContainerEl.attr('rel','tooltip');checkboxContainerEl.tooltip({title,});checkboxContainerEl.tooltip(enable?'enable':'disable');},_getRelationships:function(reportModule){const sideCtx=app.sideDrawer.currentContextDef;const appContext=app.controller.context;const currentModule=sideCtx?sideCtx.context.module:appContext.get('module');const fieldDefs=app.metadata.getModule(currentModule).fields;const subpanels=app.metadata.getLayout(currentModule,'subpanels');const relates=_.filter(fieldDefs,function(field){if(!_.isUndefined(field.type)&&(field.type==='link')){return app.data.getRelatedModule(currentModule,field.name)===reportModule;}
return false;},this);let result={};_.each(relates,function(field){result[field.name]=app.lang.get(field.vname||field.name,[currentModule,reportModule]);const linkType='link_type';const linkTypeCustom='link-type';if(field[linkType]==='one'||field[linkTypeCustom]==='one'){let oneField=false;if(field.id_name){oneField=_.first(_.filter(fieldDefs,function(targetField){return targetField.type==='relate'&&targetField.id_name===field.id_name;},this));}else{oneField=_.first(_.filter(fieldDefs,function(targetField){return targetField.type==='relate'&&targetField.link===field.name;},this));}
if(oneField&&oneField.vname){result[field.name]=app.lang.get(oneField.vname,currentModule);}}
if(!subpanels||!subpanels.components){return;}
const subpanel=_.first(_.filter(subpanels.components,function getSubpanel(component){return component.context&&component.context.link===field.name;},this));if(subpanel&&subpanel.label){result[field.name]=app.lang.get(subpanel.label,currentModule);}},this);return result;},_isNotRecordLayout:function(){return app.controller.context.get('dataView')!=='record';},_isNotFocusDrawerLayout:function(){const sideDrawerContext=app.sideDrawer.currentContextDef;return!sideDrawerContext||(sideDrawerContext&&sideDrawerContext.context.layout!=='focus');},_toggleChartFields:function(){if(this.meta.config){let xOptionsFieldset=this.getField('x_label_options');let yOptionsFieldset=this.getField('y_label_options');let showValuesField=this.getField('showValues');let showLegendField=this.getField('showLegend');let showDimensionOptions=false;let showBarOptions=false;let showLegend=true;let xOptionsLabel=app.lang.get('LBL_CHART_CONFIG_SHOW_XAXIS_LABEL');let yOptionsLabel=app.lang.get('LBL_CHART_CONFIG_SHOW_YAXIS_LABEL');switch(this.settings.get('chartType')){case'pieF':case'donutF':case'funnelF':showDimensionOptions=false;showBarOptions=false;break;case'treemapF':showLegend=false;showDimensionOptions=false;showBarOptions=false;break;case'lineF':showDimensionOptions=true;showBarOptions=false;break;case'hBarF':case'hGBarF':case'vBarF':case'vGBarF':showDimensionOptions=true;showBarOptions=true;break;default:showDimensionOptions=false;showBarOptions=false;}
if(showDimensionOptions){switch(this.settings.get('chartType')){case'hBarF':case'hGBarF':xOptionsLabel=app.lang.get('LBL_CHART_CONFIG_SHOW_YAXIS_LABEL');yOptionsLabel=app.lang.get('LBL_CHART_CONFIG_SHOW_XAXIS_LABEL');break;default:xOptionsLabel=app.lang.get('LBL_CHART_CONFIG_SHOW_XAXIS_LABEL');yOptionsLabel=app.lang.get('LBL_CHART_CONFIG_SHOW_YAXIS_LABEL');}}
if(xOptionsFieldset){xOptionRecordCell=xOptionsFieldset.$el.closest('.record-cell');if(showDimensionOptions){xOptionRecordCell.show();}else{xOptionRecordCell.hide();}
xOptionRecordCell.find('.record-label').text(xOptionsLabel);}
if(yOptionsFieldset){yOptionRecordCell=yOptionsFieldset.$el.closest('.record-cell');if(showDimensionOptions){yOptionRecordCell.show();}else{yOptionRecordCell.hide();}
yOptionRecordCell.find('.record-label').text(yOptionsLabel);}
if(showValuesField){showValuesField.$el.closest('.record-cell').toggleClass('hidden',!showBarOptions);}
if(showLegendField){showLegendField.$el.toggleClass('hidden',!showLegend);}
this.settings.set('isBarChart',!!showBarOptions);}},});});})(SUGAR.App);
/* End of File include/javascript/sugar7/plugins/ReportIntelligenceDashlet.js */
