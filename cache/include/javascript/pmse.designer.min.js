...
for(i=0;i<newLayer.ZOOMSCALES;i+=1){if(warning){cls='element-zoom-'+((i*25)+50)+'-marker adam-warning-color sicon sicon-warning-lg';}else{cls='element-zoom-'+((i*25)+50)+'-marker adam-error-color sicon sicon-warning-circle-lg';}
...
this.updateToolbar();};AdamProject.prototype.updateElement=function(updateElement){var element,i,shape,object,list;for(i=0;i<updateElement.length;i+=1){element=updateElement[i];shape=element.relatedObject;object=this.formatObject(element);list=this.getUpdateList(element.type);if(list[element.id]){$.extend(true,list[element.id],object);}else{object.action="UPDATE";list[element.id]=object;}}
...
let criteriaComponents={type:'TEMPLATE',value:data.evn_criteria};validationTools.validateAtom(criteriaComponents,element,validationTools);if(!criteria.from||!criteria.from.name||!criteria.from.id){validationTools.createWarning(element,'LBL_PMSE_ERROR_FIELD_REQUIRED','From');}
...
for(var i=0;i<criteria.length;i++){let criteriaComponents={type:actModule,module:criteria[i].field};validationTools.validateAtom(criteriaComponents,element,validationTools);}};AdamActivity.prototype.callbackFunctionForAddRelatedRecordAction=function(data,element,validationTools){validationTools.validateNumberOfEdges(1,null,1,null,element);let params=data.act_params?JSON.parse(data.act_params):null;if(params&&params.module){let baseModule=validationTools.getTargetModule();let criteria={type:'link',module:baseModule,value:params.module};validationTools.validateAtom(criteria,element,validationTools);if(params.chainedRelationship&&params.chainedRelationship.module){criteria.module=App.data.getRelatedModule(baseModule,params.module);criteria.value=params.chainedRelationship.module;validationTools.validateAtom(criteria,element,validationTools);}}
let fieldsModule=data.act_field_module;if(params&&params.chainedRelationship&&params.chainedRelationship.module){fieldsModule=params.chainedRelationship.module;}
let url=App.api.buildURL('pmse_Project/CrmData/addRelatedRecord/'+
fieldsModule+'?base_module='+validationTools.getTargetModule());let options={'bulk':'validate_element_settings'};validationTools.progressTracker.incrementTotalValidations();App.api.call('read',url,null,{success:function(form){element.validateAddRelatedRecordForm(form,data,element,validationTools);},error:function(){},complete:function(){validationTools.progressTracker.incrementValidated();}},options);};AdamActivity.prototype.validateAddRelatedRecordForm=function(form,data,element,validationTools){let i;let requiredFields;let criteria=[];if(data.act_fields){criteria=JSON.parse(data.act_fields);}
...
if(!exists){break;}}
return auxMap[type]+" # "+(i+1);};function renderProject(prjCode){var pmseCurrencies,currencies,sugarCurrencies,currentCurrency,i;adamUID=prjCode;if($('#container').length){$('#container').height($(window).height()-$('#container').offset().top);}
